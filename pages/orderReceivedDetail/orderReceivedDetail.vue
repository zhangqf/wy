<template>
	<view class="content">
		<view class="cu-form-group">
			<textarea id="context" style="width: 100%; padding: 20rpx;" :value="context" @input="bindInput" placeholder="请输入维修内容"></textarea>
		</view>
		...
		methods: {
			...
			bindOwner: function(e) {
				...
				if (msg != "") {
					...
					this.$api.saveReceives(obj).then(res => {
						 uni.hideLoading();
						if (res.code == 200) {
							uni.redirectTo({
								url: `../toBeEvaluated/toBeEvaluated`
							});
						} else {
							wx.showToast({
								title: res.msg,
