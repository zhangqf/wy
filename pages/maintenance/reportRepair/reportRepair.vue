		...
		<view class="ui-select">
			...
			<view style="width: 93%; margin: auto; margin-bottom: 20px;margin-top: 0;">
				<xfl-select ref="typeRepair" :list="repairTypeName" :clearable="false" :showItemNum="5" :listShow="false"
				 :isCanInput="false" :style_Container="'height: 44px; font-size: 16px;'" :placeholder="'请选择'" :selectHideType="'hideAll'">
				</xfl-select>
			</view>
		</view>
		...
		<view class="ui-select">
			...
			<view style="width: 93%; margin: auto; margin-bottom: 20px;margin-top: 0;">
				<xfl-select ref="xfl" :list="repairListName" :clearable="false" :showItemNum="5" :listShow="false" :isCanInput="false"
				 :style_Container="'height: 44px; font-size: 16px;'" :placeholder="'请选择'" :selectHideType="'hideAll'">
				</xfl-select>
			</view>
		</view>

		<view class="cu-form-group">
			<textarea style="width: 100%;padding: 16rpx;" id="context" :value="context" @input="bindInput" placeholder="请输入报修内容"></textarea>
		</view>
	...
	export default {
		...
		methods: {
			chooseRepairListName(data, listName) {
				const arrName = [];
				data.map((item, index) => {
					arrName.push(item.repairname || item.warrname)
				})
				console.log(arrName)
				return arrName;
			},
			chooseRepairId(data, current) {
				data.map((item, index) => {
					if (item.repairname) {
						if (item.repairname == current) {
							this.repairid = item.id
						}
					} else {
						if (item.warrname == current) {
							this.warrid = item.id
						}
					}
				})
